...
trigger_ocp_ingest() {
  ...
  if [[ -n $UUID ]]; then
    ...
    check_has_data() {
        local source_name=$1
        response=$(curl -s http://localhost:8000/api/cost-management/v1/sources/)
...
build_azure_data() {
  ...
  log-info "Trigger Azure download..."
  trigger_download "${_download_types[@]}"
}
